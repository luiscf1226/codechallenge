    ...
    <x-slot name="content">
        ...
        @if ($this->enabled)
            @if ($showingQrCode)
                ...
                <div class="mt-4 max-w-xl text-sm text-gray-600">
                    <p class="font-semibold">
                        {{ __('Escojer llave') }}: {{ decrypt($this->user->two_factor_secret) }}
                    </p>
                </div>
        ...
        <div class="mt-5">
            @if (! $this->enabled)
                <x-confirms-password wire:then="enableTwoFactorAuthentication">
                    <x-button style="background-color: #2B70FA !important; border-color: #2B70FA !important; color: white !important;" type="button" wire:loading.attr="disabled">
                        {{ __('Habilitar') }}
                    </x-button>
                </x-confirms-password>
            @else
